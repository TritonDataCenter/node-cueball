...
function NoRecordsError(name, type, ttl) {
	if (Error.captureStackTrace)
...
CueBallDNSResolver.prototype.resolve = function (domain, type, timeout) {
	...
	function onLookup(err, msg) {
		...
		/*
		 * Newer binder versions return an SOA for NODATA SRV requests
		 * that contains the TTL for the domain.
		 *
		 * Note that binder still doesn't add this to NXDOMAIN replies,
		 * and we don't fake it up below if we don't find any useful
		 * answers in the otherwise populated data.
		 */
		if (!err && msg && msg.getAnswers().length === 0) {
			var ttl = undefined;
			msg.getAuthority().forEach(function (v) {
				if (v.type === 'SOA' && v.ttl > 0) {
					ttl = v.ttl;
				}
			});
			err = new NoRecordsError(domain, type, ttl);
		}
