...
var LP_TAPS = mod_fir.genTaps(128, -0.2);
...
function CueBallConnectionPool(options) {
	...
	mod_utils.assertQueueParams(options);
	...
	this.p_lpf = new mod_fir.FIRFilter(LP_TAPS);

	this.p_idleq = new Queue();
	this.p_initq = new Queue();

	if (Array.isArray(options.queues)) {
		this.p_waiters = new Scheduler(options.queues, true);
	} else {
		this.p_waiters = new Scheduler([ {
			weight: 1,
...
CueBallConnectionPool.prototype.state_failed = function (S) {
	...
	this.p_waiters.drain(function (hdl) {
	...
	fsm.on('stateChanged', function (newState) {
		...
		if (newState === 'idle' && fsm.isInState('idle')) {
			...
			while (self.p_waiters.length > 0) {
				var hdl = self.p_waiters.dequeue();
				if (hdl !== null) {
					hdl.try(fsm);
					return;
				}
...
CueBallConnectionPool.prototype.claim = function (options, cb) {
	...
	mod_assert.optionalNumber(options.queue, 'options.queue');
	...
	queue = this.p_waiters.getQueue(options.queue);
	timeout = this.p_waiters.getTimeout(queue, options.timeout);
	...
	function tryNext() {
		...
		self.p_waiters.enqueue(queue, handle);
