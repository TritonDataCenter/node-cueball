...
function CueBallConnectionSet(options) {
	...
	if (options.collector === undefined || options.collector === null) {
		this.cs_collector = mod_artedi.createCollector();
	} else {
		this.cs_collector = options.collector;
	}

	this.cs_collector.counter({
		name: 'cueball_error_events',
...
CueBallConnectionSet.prototype._incrCounter = function (counter) {
	var errors = this.cs_collector.getCollector('cueball_error_events');
	errors.increment({
