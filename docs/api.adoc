...
- `options` -- Object, with keys:
  ...
  * `targetClaimDelay` -- optional Number, a target time for latency to claim
    accept/timeout, done through Adaptive Queue Management; this option is
    mutually exclusive with `queues`
  * `queues` -- optional Array of Object, a specification of different queues
    to place claims into, where each queue specification contains one of:
    - `weight` -- Number, the weight this queue's checkout durations should have
      relative to the others (two queues with a weight of 1 would hold handles
      for an approximately equal amount of time)
    - `targetClaimDelay` -- optional Number, the target claim latency for this
      queue
...
- `options` -- optional Object, with keys:
  * `queue` -- Number, if `queues` was provided when constructing the pool,
    then this option must be passed in order to specify which of the queues
    the claim should wait in
